...
st.caption("Created : Aman, Yedukrishnan")
...
if st.sidebar.button("Ananlyze Video"):
    st.video(url, format='video/mp4')
    with st.spinner("Learning, please wait while we analyze the video..."):
        ...
        if text_path is not None:
            ...
            st.sidebar.error("Failed to process the video. Please check if the URL is valid and try again.")
            st.error("Error: Could not process the video. Make sure ffmpeg is installed and the URL is valid.")
